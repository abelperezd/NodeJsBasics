...
async function main() {
    ...
    let devolusion = await hablar("primer blabla");
    await hablar(devolusion);
    hablar("sin await"); // esta se ejecuta y no esperamos a que termine, pasamos a la siguiente linea
    await hablar(devolusion);
    await adios(nombre);
